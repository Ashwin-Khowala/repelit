...
//   const setProjectName = useSetAtom(projectName);
...
//   if (viewMode === 'list') {
//     return (
//       <div className="bg-gray-900/50 backdrop-blur-sm rounded-lg border border-gray-800/50 p-4 hover:bg-gray-800/40 hover:border-gray-700/50 transition-all duration-200 group"  >
...
export const ProjectCard = ({ name, username, language, createdAt, lastModified, viewMode }: Project & { viewMode: 'grid' | 'list' }) => {
  ...
  const handleClick = () => {
    console.log('Setting project name:', name); // Debug log
    console.log('User email:', userEmail); // Debug log
    
    setProjectName(name);
    if (userEmail) {
      console.log('Navigating to:', `/project/${userEmail}/${name}`); // Debug log
      router.push(`/project/${userEmail}/${name}`);
    } else {
      console.warn("User email is not available yet");
    }
  ...
  if (viewMode === 'list') {
    return (
      <div className="bg-gray-900/50 backdrop-blur-sm rounded-lg border border-gray-800/50 p-4 hover:bg-gray-800/40 hover:border-gray-700/50 transition-all duration-200 group cursor-pointer"
        ...
        <div className="flex items-center justify-between">
          ...
          <button 
            className="px-4 py-2 bg-blue-600/20 text-blue-300 hover:bg-blue-600/30 hover:text-blue-200 rounded-md transition-all duration-200 border border-blue-600/30 hover:border-blue-500/50"
            onClick={(e) => {
  ...
  return (
    <div className="bg-gray-900/50 backdrop-blur-sm rounded-lg border border-gray-800/50 p-6 hover:bg-gray-800/40 hover:border-gray-700/50 transition-all duration-200 group cursor-pointer"
         onClick={handleClick}>
      <div className="flex items-start justify-between mb-4">
        <h3 className="font-semibold text-gray-100 text-lg group-hover:text-white transition-colors">{name}</h3>
        <button 
          className="text-blue-400 hover:bg-blue-600/20 rounded-md p-2 transition-all duration-200 hover:text-blue-300 border border-transparent hover:border-blue-600/30"
          onClick={(e) => {
            e.stopPropagation(); // Prevent triggering parent onClick
            // Add any specific button logic here if needed
          }}>
          <Plus className="w-4 h-4" />
        </button>
      </div>
